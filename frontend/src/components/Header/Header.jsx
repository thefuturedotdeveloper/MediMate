...
const Header = (props) => {
  ...
  const RenderButton = () => {
    if (props.value.isLoggedin === true) {
      return (
        <Link to='/profile'>
          <button className='bg-primaryColor py-2 px-6 text-white font-[600] h-[44px] flex items-center justify-center rounded-[50px]'>My Profile</button>
        </Link>
      )
    } else {
      return (
    ...
    <header className='header flex items-center' ref={headerRef}>
      <div className='container'>
        <div className='flex items-center justify-between'>
          {/* ========= logo ======= */}
          <div>
            <img src={logo2} alt="" className='w-[45%]'/>
          </div>
